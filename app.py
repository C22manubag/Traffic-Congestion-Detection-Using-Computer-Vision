...
st.caption("Analyze traffic videos using YOLO-based vehicle detection and motion analytics.")
...
if uploaded_video is not None:
    ...
    # Display compact video preview
    st.video(tfile.name, format="video/mp4", start_time=0)

    st.info("⏳ Initializing YOLO model and processing video...")
    time.sleep(1)
    ...
    final_status = "🟢 Free Flow" if avg_motion < 10 else "🔴 Traffic" if avg_motion > 25 else "🟡 Moderate"

    col1, col2, col3 = st.columns(3)
    col1.metric("Avg Vehicle Count", f"{avg_motion:.1f}")
    col2.metric("Avg Detection Confidence", f"{avg_conf:.2f}")
    col3.metric("Final Status", final_status)

    st.markdown(f"### 🧠 Overall Assessment: {final_status}")
